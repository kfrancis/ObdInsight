...
> **New finding (2026-07-18, discovered building M0.3): ElmFramer.ReadUntilAsync drops data after the delimiter.** `ElmFramer.cs` `ReadUntilAsync` returns at the first delimiter and discards any remaining bytes already read into its local buffer — if one transport read contains more than one monitoring line (burst BLE notification), every frame after the first in that chunk is silently lost. Reproduced deterministically with the replay transport (hung the monitoring test until the replay was made to chunk reads at CR). Severity: Medium (monitoring data loss under burst; request/response path unaffected since it reads to `>`). Fix direction: carry over post-delimiter remainder into a persistent buffer inside `ElmFramer`. Needs its own change + tests; not fixed in M0.3 (out of scope, touches hardware-facing behavior).
...
| Task | Description | Files | Acceptance | Effort | Risk | Deps |
|---|---|---|---|---|---|---|
| M2.1 Replace parser re-implementations | Point BMS/VIN/ISO-TP tests at production `LeafAze0Bms`, `IsoTpParser`, `ElmParsing` via ReplayElmTransport; delete `BmsParsingHelpers` parsing copies | `tests/ObdInsight.Tests.Base/LeafBmsParsingHelpers.cs`, unit test files | No parser logic defined under `tests/`; suite green | L | Medium (may surface real prod/copy divergence — good) | M0.3 |
| M2.2 Extract Nissan wakeup from ElmSession | Pluggable wakeup/probe strategy (e.g., `IEcuWakeupStrategy` supplied by vehicle profile); Leaf strategy moves next to Leaf code | `ElmSession.cs:481-564`, new interface, `NissanLeaf.cs` | No `Nissan`/`Leaf` tokens in `Communication/`; replay test covers wakeup path | M | Medium | M0.3 |

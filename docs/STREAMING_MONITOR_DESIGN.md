...
| P2 — **typed layer DONE 2026-07-18; capability pilots pending** | Done: generator conditionally implements `ObdInsight.Core.Protocols.ICanFrame<TSelf>` + `FrameCanId` when the compilation defines the interface (netstandard2.0 generator cannot host static abstracts; interface lives in Core; interface-free compilations byte-identical — snapshot-verified); `CanMonitor.Subscribe<T>()`/`TryGetLatest<T>()` extensions; replay tests decode production `BatteryFrame_1DB_AZE0` through the full stack. Pending: migrate `LeafAze0Hvac` + `LeafAze0MotorController` to view the shared monitor; console dashboard on streams. | L | Consumer API shape ✔; migration pattern pending |

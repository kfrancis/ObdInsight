...
namespace ObdInsight.Core.Protocols
{
    ...
    /// </summary>
    /// <remarks>
    ///     This static class contains methods for identifying adapter error messages, normalizing
    ///     response frames, and parsing Mode 01 OBD-II responses. The methods are designed to assist in processing raw text
    ///     data received from ELM327-compatible devices.
    /// </remarks>
    public static class ElmParsing
    {
        ...
        /// </summary>
        /// <remarks>
        ///     This method checks for specific keywords and patterns commonly found in adapter error
        ///     messages, such as "NO DATA", "UNABLE", "STOPPED", or "ERROR". The comparison is case-insensitive.
        /// </remarks>
        ...
        /// </summary>
        /// <remarks>
        ///     This method removes all null characters from the input and treats both carriage
        ///     return (\r) and line feed (\n) as line delimiters. Empty or whitespace-only lines are excluded from the
        ///     result.
        /// </remarks>
        /// <param name="frame">
        ///     The text to be split into lines. May contain carriage return (CR), line feed (LF), or both as line
        ///     separators, as well as null characters.
        /// </param>
        /// <returns>
        ///     An array of strings, each representing a trimmed, non-empty line from the input text. The array is empty if
        ...
        /// </summary>
        /// <remarks>
        ///     This method does not throw exceptions for invalid input. It returns false if the
        ///     input line does not match the expected format or the PID does not match. The comparison of the PID is
        ///     case-insensitive for the hexadecimal representation.
        /// </remarks>
        /// <param name="line">
        ///     The response line to parse, expected in the format "41 (pid) (data...)" with hexadecimal values separated by
        ///     spaces.
        /// </param>
        /// <param name="pid">The parameter ID (PID) to match, specified as a byte value.</param>
        /// <param name="data">
        ///     When this method returns, contains the parsed data bytes if parsing succeeds; otherwise, an empty
        ///     array.
        /// </param>
        ...
        public static bool TryParseMode01Response(string line, byte pid, out byte[] data)
        {
            ...
            if (!parts[0].Equals("41", StringComparison.OrdinalIgnoreCase))
            {
                return false;
            }
            ...
            {
                if (!byte.TryParse(parts[i], NumberStyles.HexNumber, null, out bytes[i - 2]))
                {
                    return false;
                }
            }

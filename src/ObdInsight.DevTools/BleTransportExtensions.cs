...
public static class BleTransportExtensions
{
    ...
    public static async Task WriteAsync(this WindowsBleTransport transport, string data, CancellationToken ct = default)
    {
        ...
        await ((IElmTransport)transport).WriteAsync(bytes, ct);
    }

    /// <summary>
    ///     Read from transport until a delimiter is found or timeout occurs.
    /// </summary>
    public static async Task<string> ReadUntilAsync(this WindowsBleTransport transport, string delimiter,
        TimeSpan timeout, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            {
                var bytesRead = await ((IElmTransport)transport).ReadAsync(buffer, cts.Token);
                if (bytesRead == 0)
                    break;
    ...
    {
        return transport.ReadUntilAsync(delimiter, timeout, CancellationToken.None);
    }
    ...
    {
        ((IElmTransport)transport).ClearBuffer();
    }
}

...
### Namespace Changes
All code moved to `ObdInsight.Core.*` namespaces:
- `ObdInsight` → `ObdInsight.Core.Communication.Bluetooth`
- `ObdInsight` → `ObdInsight.Core.Communication.Elm327`
- `ObdInsight` → `ObdInsight.Core.Protocols`
- `ObdInsight.Vehicles.*` → `ObdInsight.Core.Vehicles.Implementations.*`

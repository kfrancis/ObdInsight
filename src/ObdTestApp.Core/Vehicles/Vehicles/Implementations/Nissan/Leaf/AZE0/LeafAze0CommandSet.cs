...
public sealed class LeafAze0CommandSet : VehicleCommandSet
{
    public LeafAze0CommandSet(IElmSession session)
    {
        ...
        Add<IBrake>(new LeafAze0Brake(session, LeafAze0Contexts.BrakeBroadcast));
        ...
        Add<IVcm>(new LeafAze0Vcm(session, LeafAze0Contexts.VcmEvCanBroadcast, LeafAze0Contexts.VcmCarCanBroadcast));

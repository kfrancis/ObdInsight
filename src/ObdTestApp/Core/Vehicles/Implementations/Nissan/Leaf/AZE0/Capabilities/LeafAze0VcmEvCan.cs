...
/// <summary>
/// Internal helper for VCM operations on the EV-CAN bus.
/// Handles frames 0x11A, 0x1D4, 0x1F2, 0x284, 0x5A9, etc.
/// </summary>
internal sealed class LeafAze0VcmEvCan
{
    readonly IElmSession _session;
    readonly EcuContext _context;

    public LeafAze0VcmEvCan(IElmSession session, EcuContext context)
    {
        _session = session ?? throw new ArgumentNullException(nameof(session));
        _context = context ?? throw new ArgumentNullException(nameof(context));
        if (_context.CommunicationMode != EcuCommunicationMode.PassiveMonitoring)
            throw new ArgumentException("VCM EV-CAN requires PassiveMonitoring context for broadcast frames.", nameof(context));
    }

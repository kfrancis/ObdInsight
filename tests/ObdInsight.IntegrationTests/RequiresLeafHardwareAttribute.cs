...
public sealed class RequiresLeafHardwareAttribute : SkipAttribute
{
    public RequiresLeafHardwareAttribute()
        : base("Requires a Nissan Leaf + BLE OBD adapter. Set LEAF_BLE_ADDRESS to the adapter MAC to enable hardware tests.")
    {
    }

    public override Task<bool> ShouldSkip(TestRegisteredContext context)
        => Task.FromResult(!OdbTestApp.Tests.Fixtures.BleSessionFixture.HardwareRequested);
}

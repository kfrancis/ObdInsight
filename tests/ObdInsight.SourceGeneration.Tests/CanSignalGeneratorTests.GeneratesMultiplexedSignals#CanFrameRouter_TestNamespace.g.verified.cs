...
namespace TestNamespace
{
    ...
    {
        /// <summary>
        /// Attempts to parse a CAN frame with ID 0x5C0.
        /// </summary>
        public static bool TryParseHistoryFrame(int canId, ReadOnlySpan<byte> data, out HistoryFrame? result)
        {
            if (canId == 0x5C0 && data.Length >= HistoryFrame.MinimumLength)
            {
                result = HistoryFrame.Parse(data);
                return true;
            }
            result = null;
            return false;
        ...
        public static object? TryParseAny(int canId, ReadOnlySpan<byte> data)
        {
            return canId switch
            {
                0x5C0 => data.Length >= HistoryFrame.MinimumLength ? HistoryFrame.Parse(data) : null,
                _ => null
            };
        }
    }

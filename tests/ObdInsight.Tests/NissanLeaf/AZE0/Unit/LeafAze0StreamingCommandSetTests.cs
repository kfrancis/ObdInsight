...
public class LeafAze0StreamingCommandSetTests
{
    ...
    public async Task Hvac_And_BmsQuery_Interleave_OverSharedMonitor(CancellationToken token)
    {
        ...
        // First HVAC call starts the shared monitor.
        transport.Expect("ATMA", "");
        // BMS query suspends the monitor (exit + query) and resumes it (second ATMA).
        ...
        await Assert.That(commands.Monitor.IsRunning).IsTrue();
        await Assert.That(session.CurrentMode).IsEqualTo(ObdInsight.Core.Protocols.EcuCommunicationMode.PassiveMonitoring);
